...
public class Main {
						...
						do {
							...
							produtos.stream().forEach(x -> {
								if (x.getEmpresa().getId().equals(escolhaEmpresa)) {
									if(x.getQuantidade() > 0) { // Caso sem estoque, produto não será exibido.
										System.out.println(x.getId() + " - " + x.getNome());
									}								
							...
							for (Produto produtoSearch : produtos) {
								if (escolhaEmpresa == produtoSearch.getEmpresa().getId() & produtoSearch.getId().equals(escolhaProduto)) { // Adicionado condição para não adicionar produtos de outras empresas
									carrinho.add(produtoSearch);
									produtoSearch.setQuantidade(produtoSearch.getQuantidade()-1); // Adicionado retirada de quantidade de estoque.
								}
	...
	public static Venda criarVenda(List<Produto> carrinho, Empresa empresa, Cliente cliente, List<Venda> vendas) {
		...
		empresa.setSaldo(empresa.getSaldo() + total - comissaoSistema); // Adicionado a regra de retirada da comissão do saldo da empresa 
		vendas.add(venda);
		return venda;
	}
}
